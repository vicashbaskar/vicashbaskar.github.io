...
var myData = [];
  ...
  downloadasExcel() {
    var ws=XLSX.utils.json_to_sheet(myData1);
    var wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb,ws,'list');
    XLSX.writeFile(wb,'new.XLSX')
  }
  generateButton(button) {
    if (button != this.state.page)
      return (<button className="Table-btn" key={'button' + button} onClick={(button) => this.updateState(button)} >{button}</button>);
    else
      return (<button className="Table-btn active" key={'button' + button} onClick={(button) => this.updateState(button)} >{button}</button>);
  }
  ...
  view(page) {
    let start = (page - 1) * 10;
    let end = start + 10;
    myData = myData1.slice(start, end);
  }
  updateState(page,state) {
    this.view(page);
    this.setState({ page:this.props.state+this.view(2)})
  }
  generateRow(data) {
    return (<tr className="table-tr" key={'row' + data}>{data}</tr>);
  }
  displayRow(data) {

    let heading = Object.keys(data);
    let diplayEachRow = heading.map(eachKey => this.generateData(data[eachKey]));
    return this.generateRow(diplayEachRow);

  }
  generateHeader(data) {
    return (
      <th className="table-th" key={'head' + data}>{data}</th>
    )
  }
  displayHeader() {
    let headers = Object.keys(myData1[0]);
    let displayheaders = headers.map(eachhead => this.generateHeader(eachhead));
    return displayheaders;
  }
  generateData(data) {
    return (
      <td className="table-td" key={'td' + data}>{data}</td>
    )
  }
  displayData() {

    let data = myData.map(eachRow => this.displayRow(eachRow));
    return data;
  ...
  render() {
    ...
    return (
      ...
      <div className="tablebox-container">
        <button onClick={()=>this.downloadasExcel()}>Download</button>
        <div className="table-button">{this.displayButton()}</div>
        ...
        <div className="table-emptyspace"></div>
      </div>
